...
async def run():
    async with async_playwright() as p:
        try:
            ...
            order_ids = ["COV1016558588" , "COV1016558635" , "COV1016559735" , "COV1016563587" , "CEU5000001763" , "COV1016565731" , "COV1016565734"]

            for i in order_ids:

                #click on the keyword search
                await page.wait_for_selector('#fulltext', state='visible',timeout=30000)
                await page.fill("#fulltext", "" ,timeout=30000)
                ...
                #Fetch the original order id
                locator = page.locator("tr:has(th:text('Original Order No')) td")
                await locator.scroll_into_view_if_needed()
                text = await locator.text_content()
                order_id = text.strip().lstrip("#").strip() if text else None

                print("Order ID:", order_id)

                if order_id == i:
                    print("correct order" , i)
                    ...
                    print('INCORRECTTTTTTTTTTTTTT order id' , i)
